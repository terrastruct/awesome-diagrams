...
# Prefer ImageMagick 7 (magick) over ImageMagick 6 (convert)
if command -v magick >/dev/null; then
    log "ImageMagick 7 (magick) found - thumbnail generation enabled with deterministic output"
...
if [ "$diagrams_line" != "999999" ]; then
    ...
    # First, find where the actual content ends (before trailing blank lines)
    content_end=$(head -n $((diagrams_line - 1)) README.md | grep -n "." | tail -1 | cut -d: -f1)
    if [ -n "$content_end" ]; then
        head -n "$content_end" README.md > "$TEMP_FILE"
        log "Found existing Diagrams section at line $diagrams_line, preserving content up to line $content_end"
    else
        # If no content found, just copy what we have
...
# Add diagram section header with proper spacing
_echo "" >> "$TEMP_FILE"
_echo "" >> "$TEMP_FILE"
...
_generate_thumbnail() {
    ...
    if [ -n "$first_image" ]; then
        ...
        # Use magick command (ImageMagick 7) if available, otherwise fall back to convert
        # The -strip and -define png:exclude-chunks options ensure deterministic output
        if command -v magick >/dev/null 2>&1; then
            if hide magick "$first_image" -resize 150x -quality 85 -strip -define png:exclude-chunks=date,time,chrm,gama,offs,phys,srgb,bkgd,hist,exif,itxt,text,ztxt -define png:include-chunk=none +set date:create +set date:modify +set date:timestamp "$thumbnail_path"; then
            ...
            if hide convert "$first_image" -resize 150x -quality 85 -strip -define png:exclude-chunks=date,time,chrm,gama,offs,phys,srgb,bkgd,hist,exif,itxt,text,ztxt +set date:create +set date:modify +set date:timestamp "$thumbnail_path"; then
                _echo "success|$diagram_name|$diagram_project|$thumbnail_path" > "$result_file"
            else
                _echo "failed|$diagram_name|$diagram_project|" > "$result_file"
            fi
